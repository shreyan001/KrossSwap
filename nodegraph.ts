...
export default function nodegraph() {
    const graph = new StateGraph<krossState>({
        channels:{
            messages: {
    ...
    graph.addNode("initial_node", async (state:krossState ) => {
        const SYSTEM_TEMPLATE = `You are krossSwap, an AI-powered Telegram bot facilitating cross-chain cryptocurrency swaps powered by Chainflip SDK on the Perseverance testnet.
        ...
        4. Answer basic questions about krossSwap and cross-chain swaps.
    ...
    graph.addConditionalEdges("initial_node", async (state) => {
        const SYSTEM_TEMPLATE = `You are a support system for the krossSwap bot responsible for routing the conversation to either the pre-check node or continuing the conversation.
...
Remember, only respond with one of the above words.`;
    ...
    graph.addNode("precheck_node", async (state: krossState) => {
        ...
        if (validationResult === "SUCCESS") {
            const swapValues: krossState['swapValues'] = {
    ...
    graph.addNode("getQuote_node", async (state: krossState) => {
        try {
...
`;
     ...
     /* @ts-ignore */
    graph.addConditionalEdges("getQuote_node", (state:krossState) => {
