...
bot.command('start', (ctx) => {
  ...
  Hey ${username}! 👋 Welcome to krossSwap! 🚀

  We're here to make cross-chain crypto swaps as easy as chatting with a friend! 💬✨

  🔄 krossSwap uses the power of Chainflip to seamlessly swap your crypto assets across different blockchains. No complicated menus or confusing interfaces - just tell us what you want to do!
  ...
  🌈 With krossSwap, you have access to a wide range of tokens and chains supported by Chainflip. More options, more freedom!
        ...
        if (match) {
            ...
            if (!isGroup) {
                yield ctx.reply('You can check your transaction status or scan QR code to complete the transaction', telegraf_1.Markup.inlineKeyboard([
                    telegraf_1.Markup.button.webApp('Open Payment QR', `https://krossswapbot.vercel.app/?id=${depositChannelId}`)
                ]));
            }
        }
    }
